...
void balance(float pid_feedback, float yaw)
{
	speed = int(abs(pid_feedback));
	steering_speed = int(pitch_error * k_pitch);
	motor_A_speed = speed + steering_speed;
	motor_B_speed = speed - steering_speed;
	if(feedback < 0)
		go_forward();
	else
		go_backward();
...
void steering(float pitch_error)
{
	motor_A_speed ;
	motor_B_speed ; 
}
